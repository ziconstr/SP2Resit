...
async function getProfileListings(name) {
  const data = await fetchData(
    `${API_BASE}/auction/profiles/${name}/listings?_bids=true`,
  );
  return data.data;
}
...
function renderProfile(profile, listings, bids) {
  ...
  container.innerHTML = `
    ...
    <div class="rounded-2xl overflow-hidden border border-gray-200 shadow-sm bg-white mb-6">
      ${banner ? `<img src="${banner}" class="w-full h-40 object-cover" alt="banner" />` : `<div class="w-full h-40 bg-[#0063FB]"></div>`}
      <div class="px-6 pb-6">
        <div class="flex items-end gap-4 -mt-10">
          <img src="${avatar}" alt="avatar" class="w-20 h-20 rounded-full border-4 border-white shadow object-cover" onerror="this.src='https://placehold.co/100?text=?'" />
          <div class="pb-1">
    ...
    <div class="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 mb-6">
      ...
      <div class="space-y-3">
        <div>
          ...
          <input type="text" id="edit-bio" value="${profile.bio || ''}" placeholder="Your bio" class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#0063FB]" />
        </div>
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-1">Avatar URL</label>
          <input type="text" id="edit-avatar" value="${profile.avatar?.url || ''}" placeholder="https://..." class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#0063FB]" />
        </div>
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-1">Banner URL</label>
          <input type="text" id="edit-banner" value="${profile.banner?.url || ''}" placeholder="https://..." class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#0063FB]" />
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ${listings
            .map((listing) => {
              ...
              const deadline = new Date(listing.endsAt).toLocaleDateString(
                'no-NO',
              );
              return `
              <a href="/listing.html?id=${listing.id}" class="flex gap-3 border border-gray-100 rounded-xl p-3 hover:shadow transition">
                <img src="${image}" alt="${listing.title}" class="w-20 h-20 object-cover rounded-lg" onerror="this.src='https://placehold.co/400x200?text=No+Image'" />
